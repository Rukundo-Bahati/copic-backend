const socketHandler = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("new-user-add", (newUserId) => {
      ...
      socket.emit("get-users", Array.from(activeUsers.keys())); // Send only to the current user
    });
    ...
    socket.on("disconnect", () => {
      ...
      io.emit("get-users", Array.from(activeUsers.keys())); // Notify all users about the updated list
    });
